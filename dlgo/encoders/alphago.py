...
"""
Название признака       Кол.плоскостей  Описание
--------------------------------------------------------------------------------------------------------------
Цвет камней             3               Три плоскости признаков, соответствующие цвету камней, – по одной
(Stone colour)                          для текущего игрока, для его противника и для пустых точек на доске.
--------------------------------------------------------------------------------------------------------------
Еденицы (Ones)          1               Плоскость признаков, полностью заполненная единицами.
--------------------------------------------------------------------------------------------------------------
Нули (Zeros)            1               Плоскость признаков, полностью заполненная нулями.
--------------------------------------------------------------------------------------------------------------
Допустимость            1               Если ход является допустимым и не приводит к заполнению
(Sensibleness)                          «глаз» текущего игрока, то он кодируется на этой плоскости
                                        значением 1, в противном случае – значением 0.
--------------------------------------------------------------------------------------------------------------
Ходов с тех пор         8               Этот набор из восьми бинарных плоскостей показывает,
(Turns since)                           сколькими ходами ранее был совершен тот или иной ход.
--------------------------------------------------------------------------------------------------------------
Степени свободы         8               Количество степеней свободы цепочки камней, к которым относится
(Liberties)                             этот ход, разделенное на восемь бинарных плоскостей.
--------------------------------------------------------------------------------------------------------------
Степени свободы         8               Сколько степеней свободы останется в результате совершения               
после совершения                        данного хода?
хода (Liberties after
move)
--------------------------------------------------------------------------------------------------------------
Количество              8               Сколько камней противника можно захватить, совершив
захваченных камней                      этот ход?
(Capture size)                          
--------------------------------------------------------------------------------------------------------------
Количество камней       8               Сколько из ваших камней в случае совершения этого хода окажутся
в положении «атари»                     в положении «атари» и смогут быть захвачены противником на
(Self-atari size)                       следующем ходу?
--------------------------------------------------------------------------------------------------------------
Захват «лесенка»        1               Может ли этот камень быть захвачен с помощью «лесенки»?
(Ladder capture)                        
--------------------------------------------------------------------------------------------------------------
Защита от захвата       1               Может ли этот камень избежать возможных «лесенок»?
«лесенка»
(Ladder escape)
--------------------------------------------------------------------------------------------------------------
Цвет камней             1               Эта плоскость заполняется единицами, если текущий игрок
текущего игрока                         играет черными, и нулями – если белыми.

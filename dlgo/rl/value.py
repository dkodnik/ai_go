...
class ValueAgent(Agent):
    ...
    def select_move(self, game_state):
        ...
        # Повторите все законные действия.
        moves = []
        board_tensors = []
        ...
        # Значения следующего состояния с точки зрения противника.
        opp_values = self.model.predict(board_tensors)
        opp_values = opp_values.reshape(len(moves))

        # Ценности с нашей точки зрения.
        values = 1 - opp_values
    ...
    def rank_moves_eps_greedy(self, values):
        ...
        # Это ранжирует ходы от худшего к лучшему.
        ranked_moves = np.argsort(values)
        # Верните их в порядке от наилучшего к наихудшему.
